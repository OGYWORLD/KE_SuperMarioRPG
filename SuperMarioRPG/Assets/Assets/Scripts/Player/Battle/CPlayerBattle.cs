...
public class CPlayerBattle : MonoBehaviour
{
    ...
    public AudioClip acAttack = null;
    public AudioClip hammerSound = null;

    public AudioClip thunder = null;
    public AudioClip jump = null;

    public AudioClip item = null;
    ...
    IEnumerator AttackMotion()
    {
        ...
        if (btlManager.status == 21) // hammer show
        {
            playerAudio.clip = hammerSound;
            playerAudio.Play();
        ...
        if (btlManager.status == 21) // hammer hide
        {
    ...
    public void CheckActionCommand()
    {
        if (isTiming)
        {
            ...
            {
                combo.SetGauge10();
                GameManager.instance.gauge += 9; // attack Action Command Update
    ...
    void EndAttck()
    {
        if (btlManager.isLeftMstDead && btlManager.isRightMstDead)
        {
            btlManager.status = 6; // hide Command Menu
        }
        else
        {
    ...
    IEnumerator MagicAttackMotion(float _t)
    {
        ...
        if (isScAtkCmd)
        {
            ...
            printNum.PrintNum(GameManager.instance.members[btlManager.curPlrTurn].m_magicAttack[btlManager.status % 10].m_attack * 2, btlManager.curAttack); // print Damage Num
            monApr.ShowACParticle(); // Show Monster ActionCommand Particle
            yield return new WaitForSeconds(1.0f);
            monApr.HideACParticle(); // Hide Monster ActionCommand Particle
        }
    ...
    IEnumerator ShowItemUse()
    {
        ...
        itemParticle[curItem].GetComponent<Transform>().position = btlManager.plrPos[btlManager.curPlrTurn] + new Vector3(0f, 2f, -0.3f);
        itemParticle[curItem].SetActive(true);

        playerAudio.clip = item;
        playerAudio.Play();
        ...
        // cloud
        itemParticle[0].GetComponent<Transform>().position = btlManager.plrPos[btlManager.curPlrTurn] + new Vector3(0f, 2f, -0.3f);
        itemParticle[0].SetActive(true);

        playerAudio.clip = cloud;
        playerAudio.Play();
        ...
        {
            playerAudio.clip = mushroom;
            playerAudio.Play();

            itemParticle[curItem + 2].GetComponent<Transform>().position = btlManager.plrPos[(int)btlManager.whoUseMushroom] + new Vector3(0f, 0.5f, -0.3f);
        }
        else
        {
            playerAudio.clip = honeySyrup;
            playerAudio.Play();

            itemParticle[curItem + 2].GetComponent<Transform>().position = btlManager.plrPos[btlManager.curPlrTurn] + new Vector3(0f, 0.5f, -0.3f);
        }
