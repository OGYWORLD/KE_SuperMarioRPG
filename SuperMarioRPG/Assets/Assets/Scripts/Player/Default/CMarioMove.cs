...
public class CMarioMove : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameManager.instance.curStage = GameManager.instance.stageName[curScene.name];

        if(curScene.name == "m01i1f1")
        {
           // DO Nothig in Store
        }
        else if (GameManager.instance.beforeSceneName != curScene.name)
        {
            // Monster List Init
            GameManager.instance.monsterAtv.Clear();
        ...
        {
            gameObject.transform.position = GameManager.instance.curMarioPos;
        }
    ...
    void Update()
    {
        ...
        if (!isTalk && !GameManager.instance.isMenu)
        {
            isBeDetectedBottom();
            InputMove();
            Jump();
    ...
    void isBeDetectedBottom()
    {
        if (Physics.Raycast(mario.transform.position, -mario.transform.up, out RaycastHit hit, 0.1f))
        {
            ...
            if (hit.collider.name == "1101_flower_mdl") // flower roation
            {
                jumpPowerHeight = 5.0f + curMarioY;
                mario.transform.Rotate(Vector3.up * 1f);
            }
        }
        else
        {
