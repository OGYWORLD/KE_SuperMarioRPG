...
public class CBattleManager : MonoBehaviour
{
    ...
    // Playable Dead Info
    public Vector3[] plrPos { get; set; } = { new Vector3(0.55f, 0f, 3.3f), new Vector3(-1.74f, 0f, 2.91f) };
    ...
    void Start()
    {
        GameManager.instance.isToucehd = false;
    ...
    public void CheckMstDead()
    {
        ...
        {
            if(GameManager.instance.btlMoster == "hammerBro")
            {
                GameManager.instance.isHammerBroEvent = 1;
                GameManager.instance.isHammerBroDead = true;
            }
            status = 6; // WIN
